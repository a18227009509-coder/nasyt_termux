...
#检查安装函数
apt_install() {
    apt_install_app="$*" #读取要安装的软件包
    if command -v $apt_install_app >/dev/null 2>&1; then
        echo -e "$green ◉ $apt_install_app已经安装，跳过安装步骤。$color"
    else 
        echo "正在安装$apt_install_app"
        apt install $apt_install_app
        if [ $? -ne 0 ]; then
...
must_apt_install() {
    echo "正在检查必备软件包安装"
    apt_install curl -y
    apt_install proot-distro -y
    apt_install neofetch -y
    apt_install figlet -y
    apt_install wget -y
    apt_install git -y
    apt_install make -y
    apt_install cmake -y
                            ...
                            do
                                ...
                                case $qemu_shell_xz in
                                    1)
                                        ...
                                        clear
                                        apt_install qemu-system-x86-64-headless qemu-utils -y
